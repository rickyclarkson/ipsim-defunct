...
import anylayout.{Size, AnyLayout, LayoutContext}
...
object CustomJOptionPane {
 val padding: Int = 10
 val paddingFunction = (context: LayoutContext) => padding
 ...
 def showYesNoCancelDialog(message: String, title: String)(implicit frame: JFrame) = {
  ...
  val sizeCalculator=Size( {
   ...
   padding*3+messageLabel.getPreferredSize.height+cancelButton.getPreferredSize.height)
 ...
 def showLabelsAndConfirmation(frame: JFrame, title: String, message: String, choices: Array[String], theDefault: Int, toConfirm: String) = {
  ...
  val calculator=Size( {
   val messageSize=messageLabel.getPreferredSize
  ...
  def add(one: LayoutContext => Int, two: LayoutContext => Int) =
   (c: LayoutContext) => one(c) + two(c)
  ...
  val firstButtonConstraint = buildConstraint.setLeft(paddingFunction).setTop(_.layoutInfoFor(confirmationLabel).getFarOffset + padding).setWidth(preferredSize).setHeight(preferredSize)
 ...
 def showNonModalMessageDialog(title: String, component: Component)(implicit frame: JFrame): Unit = {
  ...
  AnyLayout.useAnyLayout(container, 0.5F, 0.5F, Size(component.getPreferredSize.width + padding * 2, padding * 3 + component.getPreferredSize.height + okButton.getPreferredSize.height), null)
...
object LabelledTextField {
 ...
 def createLabelledTextField2[T <: JTextField](text: String, field: T) = {
  ...
  useAnyLayout(panel, 0.5f, 0.5f, Size(field.getPreferredSize.width + label.getPreferredSize.height + 5, Math.max(field.getPreferredSize.height, label.getPreferredSize.height)), ConstraintUtility.typicalDefaultConstraint(Runnables.throwRuntimeException))
  val constant: LayoutContext => Int = c => 0
