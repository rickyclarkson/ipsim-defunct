...
object InitialDialog {
 ...
 def initialDialog(implicit random: Random, frame: JFrame, network: Network): JDialog = {
  ...
  AnyLayout.useAnyLayout(contentPane, 0.5F, 0.5F, Size( {
   ...
   (List(introLabel, freeformRadioButton, practiceTestRadioButton, takeTestRadioButton, okButton) map (_.getPreferredSize.height) reduceLeft ((_: Int)+(_: Int))) + padding * 5 }), ConstraintUtility typicalDefaultConstraint new Runnable { def run = throw null } )
  ...
  val freeformConstraint = ConstraintBuilder.buildConstraint setLeft (i => padding) setTop (_.layoutInfoFor(introLabel).getFarOffset + padding * 3)  setWidth ConstraintBuilder.preferredSize setHeight ConstraintBuilder.preferredSize
  ...
  val takeTestConstraints = ConstraintBuilder.buildConstraint setLeft (layoutContext => layoutContext.parentSize.intValue - 10 - layoutContext.preferredSize.intValue) setTop (layoutContext => layoutContext.layoutInfoFor(practiceTestRadioButton).getOffset) setWidth ConstraintBuilder.preferredSize setHeight ConstraintBuilder.preferredSize
...
object ConnectivityTestDialog { def apply() = {
 ...
 AnyLayout.useAnyLayout(dialog.getContentPane, 0.5f, 0.5f, Size(700, 400), null)
...
object ExceptionReportDialog { def handle(exception: Throwable)(implicit frame: JFrame, network: Network) = {
 ...
 val constraints = PercentConstraintsUtility newInstance dialog.getContentPane
 AnyLayout useAnyLayout (dialog.getContentPane, 0.5F, 0.5F, constraints.size,
...
case class PingDialog(computer: Computer)(implicit frame: JFrame, network: Network) {
 ...
 AnyLayout useAnyLayout (wrapped.getContentPane, 0.5F, 0.5F, Size(600, 400),
                         ConstraintUtility typicalDefaultConstraint Runnables.throwRuntimeException)
...
object TracerouteDialog { def newTracerouteDialog(computer: Computer)(implicit network: Network, frame: JFrame) = {
 ...
 AnyLayout useAnyLayout(pane, 0.5F, 0.5F, Size(200, 100), typicalDefaultConstraint(throwRuntimeException))
...
object ProblemDialog { def createProblemDialog(implicit frame: JFrame, context: NetworkContext) = {
 ...
 dialog centreOnParent frame
 val constraints = PercentConstraintsUtility newInstance dialog.getContentPane
 useAnyLayout(dialog getContentPane, 0.5F, 0.5F, constraints size, typicalDefaultConstraint(throwRuntimeException))
...
object AddDefaultRouteDialogUtility { def newInstance(computer: Computer)(implicit frame: JFrame, network: Network) = {
 ...
 val pane = dialog.getContentPane
 val constraints = PercentConstraintsUtility newInstance pane
 useAnyLayout(pane, 0.5F, 0.5F, constraints size, ConstraintUtility typicalDefaultConstraint Runnables.throwRuntimeException)
...
object RoutingTableEntryEditDialog {
 ...
 def createRouteEditor(dialog: JDialog, computer: Computer, entry: RouteInfo, realRoute: Option[Route], parent: Option[RoutingTableDialog])(
  ...
  val constraints = PercentConstraintsUtility newInstance dialog.getContentPane
  useAnyLayout(dialog getContentPane, 0.5F, 0.5F, Size(400, 200), typicalDefaultConstraint(Runnables.throwRuntimeException))
...
object ScrapbookDialogUtility {
 def createScrapbook(frame: JFrame)(implicit network: Network) = {
  ...
  AnyLayout useAnyLayout (panel, 0.5f, 0.5f, constraints size, typicalDefaultConstraint(Runnables throwRuntimeException))
...
object ScrapbookElement { def createElement = {
 val panel = new JPanel
 val constraints = PercentConstraintsUtility newInstance panel
 AnyLayout useAnyLayout (panel, 0.5f, 0.5f, constraints size, typicalDefaultConstraint(throwRuntimeException))
